...
final class PerformanceComparisonTests: XCTestCase {
    ...
    func testContextPoolingPerformance() async throws {
        // Demonstrates context pooling memory and performance benefits
        
        let iterations = 1000
        let pool = CommandContextPool(maxSize: 50)
        
        // Measure without pooling
        let withoutPoolTime = await measureAsync {
            var contexts: [CommandContext] = []
            for _ in 0..<iterations {
                contexts.append(CommandContext(
                    metadata: StandardCommandMetadata(userId: "test")
                ))
            }
            contexts.removeAll() // Force deallocation
        }
        
        // Measure with pooling
        let withPoolTime = await measureAsync {
            var contexts: [PooledCommandContext] = []
            for _ in 0..<iterations {
                contexts.append(pool.borrow(
                    metadata: StandardCommandMetadata(userId: "test")
                ))
            }
            contexts.removeAll() // Returns to pool
        }
        
        // Calculate improvement
        let improvement = ((withoutPoolTime - withPoolTime) / withoutPoolTime) * 100
        print("Context pooling is \(String(format: "%.1f", improvement))% faster")
        
        // Check pool statistics
        let stats = pool.getStatistics()
        print("Pool hit rate: \(String(format: "%.1f", stats.hitRate * 100))%")
        
        XCTAssertLessThan(withPoolTime, withoutPoolTime)
        XCTAssertGreaterThan(stats.hitRate, 0.8) // Should have high hit rate
    }
    ...
    func testCachedMiddlewarePerformance() async throws {
        ...
        let pipeline = try await PipelineBuilder(handler: handler)
            .with([expensiveMiddleware.cached(ttl: 60)])
            .build()
