...
final class SendableClosureTests: XCTestCase {
    ...
    func testPipelineBuilderDSLConditionalClosure() async throws {
        // This test verifies that Sendable closures work properly with PipelineBuilder
        let handler = MockCommandHandler()
        
        // Create a middleware that uses a @Sendable closure
        let counter = TestActor<Int>(0)
        let countingMiddleware = ConditionalMiddleware { @Sendable command, context, next in
        ...
        _ = await builder.with(countingMiddleware)
        let pipeline = try await builder.build()
        
        // Execute commands
        let command = MockCommand(value: 42, shouldFail: false)
        let result1 = try await pipeline.execute(command, context: CommandContext())
        XCTAssertEqual(result1, "Result: 42")
        
        let result2 = try await pipeline.execute(command, context: CommandContext())
        XCTAssertEqual(result2, "Result: 42")
    ...
    func testParallelMiddlewareContextIsolation() async throws {
        ...
        let builder = PipelineBuilder(handler: handler)
        _ = await builder.with(ParallelMiddlewareWrapper(middlewares: [middleware1, middleware2]))
        let pipeline = try await builder.build()
...
private struct ConditionalMiddleware: Middleware, Sendable {
    let priority: ExecutionPriority = .processing
    let closure: @Sendable (any Command, CommandContext, @Sendable (any Command, CommandContext) async throws -> Any) async throws -> Any
    
    init(closure: @escaping @Sendable (any Command, CommandContext, @Sendable (any Command, CommandContext) async throws -> Any) async throws -> Any) {
