...
final class PrioritizedMiddlewareIntegrationTests: XCTestCase {
    ...
    func testSecurePipelineOrdering() async throws {
        ...
        let builder = PipelineBuilder(handler: TestHandler())
        // Add security middleware manually in the correct order
        _ = await builder.with(TrackingMiddleware(name: "validation", tracker: tracker))
        _ = await builder.with(TrackingMiddleware(name: "sanitization", tracker: tracker))
        _ = await builder.with(TrackingMiddleware(name: "auth", tracker: tracker))
        _ = await builder.with(TrackingMiddleware(name: "authz", tracker: tracker))
        _ = await builder.with(TrackingMiddleware(name: "log", tracker: tracker))
