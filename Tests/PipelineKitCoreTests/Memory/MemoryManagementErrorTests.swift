...
final class MemoryManagementErrorTests: XCTestCase {
    ...
    func testObjectPoolExhaustion() async throws {
        ...
        // When - Acquire objects beyond pool size
        let obj1 = await pool.acquire()
        let obj2 = await pool.acquire()
        let obj3 = await pool.acquire()
        let obj4 = await pool.acquire() // This creates a new object beyond maxSize
        
        // Then - Pool should create new objects, not block
        XCTAssertNotNil(obj4, "Pool should create new objects when exhausted")
        
        // Verify statistics
        let stats = await pool.statistics
        XCTAssertEqual(stats.allocations, 4, "Should have allocated 4 objects")
        XCTAssertEqual(stats.acquisitions, 4, "Should have 4 acquisitions")
        
        // Cleanup
        await pool.release(obj1)
        await pool.release(obj2)
        await pool.release(obj3)
        await pool.release(obj4)
        
        // After release, pool should only keep maxSize objects
        let finalStats = await pool.statistics
        let availableCount = await pool.availableCount
        XCTAssertLessThanOrEqual(availableCount, 3, "Pool should respect maxSize after release")
    }
    ...
    func testMemoryPressureHandlerFailure() async throws {
        ...
        let cleanupAttempts = await counter.value
    ...
    func testMemoryLeakDetection() async throws {
        ...
        class LeakyObject {
            ...
            let data = [Int](repeating: 0, count: 100_000) // Increase size to ~800KB per object
        ...
        // Create enough to exceed 100MB threshold
        for _ in 0..<150 {
            _ = LeakyObject()
        ...
        case .leak:
            ...
            break
        case .growing:
            // Also acceptable - indicates memory growth
            break
        default:
            XCTFail("Should detect memory leak pattern, got \(pattern)")
        }
    }
    ...
    func testResourceCleanupOnDeinit() async throws {
        ...
        final class TrackingPool: @unchecked Sendable {
            ...
            init() {
                pool = ObjectPool<ExpensiveObject>(
                    ...
                    factory: { ExpensiveObject() },
                    reset: { _ in
                        // Note: reset is called when objects are returned to pool,
                        // not when pool is deallocated
                    }
                )
            }
        }
...
final class ExpensiveObject: Sendable {
    let id = UUID()
