...
final class PerformanceMiddlewareTests: XCTestCase {
    ...
    func testPerformanceWithoutCollector() async throws {
        ...
        let performanceData = await context.get(PerformanceMeasurement.self, for: "performance_measurement")
        XCTAssertNotNil(performanceData)
        XCTAssertEqual(performanceData?.commandName, "PerfTestCommand")
        XCTAssertTrue(performanceData?.isSuccess ?? false)
    }
}
