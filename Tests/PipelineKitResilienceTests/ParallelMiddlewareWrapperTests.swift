...
final class ParallelMiddlewareWrapperTests: XCTestCase {
    ...
    func testContextIsolation() async throws {
        ...
        let value1: String? = await context.get(TestContextKeys.dynamic("key1"))
        let value2: String? = await context.get(TestContextKeys.dynamic("key2"))
        let value3: String? = await context.get(TestContextKeys.dynamic("key3"))
