...
final class TypeSafeMetricTests: XCTestCase {
    ...
    func testCreateTimer() {
        let timer = Metric<Timer>.timer("test.timer", duration: 0.123, tags: ["method": "GET"])
    ...
    func testTimerToSnapshot() {
        let timer = Metric<Timer>.timer("timer.test", duration: 0.5)
    ...
    func testMetricKindTypes() {
        ...
        XCTAssertEqual(Timer.type, .timer)
    ...
    func testZeroValues() {
        ...
        let timer = Metric<Timer>.timer("test", duration: 0.0)
