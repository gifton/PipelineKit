...
final class SamplingTests: XCTestCase {
    ...
    func testCounterScaling() async {
        ...
        XCTAssertEqual(sampledCounter?.value, 100.0, accuracy: 0.01)  // 10.0 * 10
    }
...
/// Testable exporter that exposes formatted metrics
private actor TestableStatsDExporter: StatsDExporter {
    private var lastFormattedMetric: String?
    
    override func record(_ snapshot: MetricSnapshot) async {
        // Store the formatted metric for testing
        let (shouldSample, rate) = shouldSample(snapshot)
        guard shouldSample else { return }
        ...
        lastFormattedMetric = formatMetric(adjustedSnapshot, sampleRate: rate)
        await super.record(snapshot)
    }
    
    func getLastFormattedMetric() -> String? {
        lastFormattedMetric
    }
    
    // Expose the private sampling method for testing
    private func shouldSample(_ snapshot: MetricSnapshot) -> (sample: Bool, rate: Double) {
        // This would need to be exposed or duplicated for testing
