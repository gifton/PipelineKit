    ...
    func testTimeoutErrorPropagation() async throws {
        ...
        struct TimeoutThrowingMiddleware: Middleware {
            ...
            func execute<T: Command>(
                ...
                throw PipelineError.cancelled(context: "Timeout in TestMiddleware for command \(String(describing: T.self)) after 1.0s")
            }
        }
        ...
        do {
            ...
            if case .cancelled(let context) = error {
                ...
                XCTAssertTrue(context?.contains("TestMiddleware") ?? false)
            } else {
