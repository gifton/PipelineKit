    ...
    private func calculateRequestedCount(for request: ExhaustionRequest) throws -> Int {
        ...
        case .percentage(let percentage):
            guard percentage >= 0 && percentage <= 1.0 else {
                throw PipelineError.simulation(reason: .exhaustion(.invalidAmount("Percentage must be between 0 and 1")))
            }
        ...
        case .bytes(let bytes):
            ...
            default:
                throw PipelineError.simulation(reason: .exhaustion(.invalidAmount("Bytes specification not supported for \(request.resource)")))
            }
        }
    }
