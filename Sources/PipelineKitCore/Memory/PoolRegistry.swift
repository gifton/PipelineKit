...
public actor PoolRegistry {
    ...
    private init() {
        ...
        guard Self.cleanupInterval > 0 else {
            fatalError("PoolRegistry: cleanupInterval must be positive, got \(Self.cleanupInterval)")
        }
    ...
    /// This is nonisolated so it can be called synchronously from deinit
    ...
    /// Synchronous unregister for use in deinit contexts
    /// Uses a semaphore with timeout to prevent hanging
    nonisolated public func unregisterSync(id: ObjectIdentifier) {
        ...
        if result == .timedOut {
            ...
            #if DEBUG
            print("Warning: unregisterSync timed out for pool \(id)")
            #endif
        }
    }
