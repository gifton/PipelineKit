...
/// ## Thread Safety
///
/// PooledObject is thread-safe through the use of NSLock to protect the
/// `isReturned` flag. The wrapped object itself must be Sendable to ensure
...
public extension ObjectPool {
    ...
    func acquirePooled() async -> PooledObject<T> {
        let object = acquire()
