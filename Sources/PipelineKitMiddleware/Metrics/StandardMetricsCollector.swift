...
actor StandardMetricsCollector: MetricsCollector {
    static let shared = StandardMetricsCollector()
    ...
    func recordLatency(_ duration: TimeInterval, for operation: String, tags: [String: String]) {
    ...
    func incrementCounter(_ name: String, tags: [String: String]) {
        let key = makeKey(name, tags: tags)
        counters[key, default: 0] += 1
    }
    
    func recordGauge(_ value: Double, for name: String, tags: [String: String]) {
        let key = makeKey(name, tags: tags)
        gauges[key] = value
    }
    
    /// Get metric statistics
    func getStats() -> MetricsSnapshot {
        MetricsSnapshot(
            latencies: latencies,
            counters: counters,
            gauges: gauges
...
public struct MetricsSnapshot: Sendable {
    ...
    func percentiles(for metric: String, percentiles: [Double] = [0.5, 0.95, 0.99]) -> [Double: TimeInterval] {
