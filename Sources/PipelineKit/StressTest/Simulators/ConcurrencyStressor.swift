...
public actor ConcurrencyStressor: MetricRecordable {
    ...
    public func createActorContention(
        ...
        guard state == .idle else {
            throw ConcurrencyError.invalidState(current: "\(state)", expected: "idle")
        }
        ...
        guard await safetyMonitor.canCreateActors(count: actorCount) else {
            ...
            throw ConcurrencyError.safetyLimitExceeded(
    ...
    public func createLockContention(
        ...
        do {
            ...
            state = .idle
            await recordPatternFailure(.patternFail, error: error, tags: ["pattern": "lock_contention"])
            throw error
        }
    }
        ...
        do {
            try await withThrowingTaskGroup(of: Void.self) { group in
                ...
                for i in 0..<taskPairs {
                    ...
                    group.addTask { [weak self] in
                        ...
                        guard acquiredA else {
                            throw ConcurrencyError.resourceExhausted(type: "lock_a")
                        }
                        ...
                        guard acquiredB else {
                            await lockA.release()
                            throw ConcurrencyError.resourceExhausted(type: "lock_b")
                        }
                ...
                group.addTask {
                    ...
                    throw ConcurrencyError.resourceExhausted(type: "deadlock_timeout")
                }
            }
        } catch {
...
public enum ConcurrencyError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidState(let current, let expected):
            return "Invalid stressor state: \(current), expected \(expected)"
        ...
        case .resourceExhausted(let type):
            return "Resource exhausted: \(type)"
        }
    }
}
