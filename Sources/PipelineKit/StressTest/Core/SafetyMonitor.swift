...
public actor DefaultSafetyMonitor: SafetyMonitor {
    ...
    public func allocateActor() async throws -> ResourceHandle<Never> {
        guard !isShutdown else { throw SafetyResourceError.shutdownInProgress }
        ...
        guard await canCreateActors(count: 1) else {
            ...
            throw SafetyResourceError.limitExceeded(type: .actor, requested: 1)
        }
    ...
    public func allocateTask() async throws -> ResourceHandle<Never> {
        ...
        guard await canCreateTasks(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .task, requested: 1)
        }
    ...
    public func allocateLock() async throws -> ResourceHandle<Never> {
        ...
        guard await canAcquireLocks(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .lock, requested: 1)
        }
    ...
    public func allocateFileDescriptor() async throws -> ResourceHandle<Never> {
        ...
        guard await canOpenFileDescriptors(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .fileDescriptor, requested: 1)
        }
    ...
    public func allocateMemoryMapping(size: Int) async throws -> ResourceHandle<Never> {
        ...
        guard await canAllocateMemory(size) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .memoryMapping, requested: size)
        }
    ...
    public func allocateSocket(type: SocketType) async throws -> ResourceHandle<Never> {
        ...
        guard await canOpenFileDescriptors(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .socket, requested: 1)
        }
    ...
    public func allocateDiskSpace(size: Int) async throws -> ResourceHandle<Never> {
        ...
        guard await canOpenFileDescriptors(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .diskSpace, requested: size)
        }
    ...
    public func allocateThread() async throws -> ResourceHandle<Never> {
        ...
        guard await canCreateTasks(count: 1) else {
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .thread, requested: 1)
        }
    ...
    public func allocateProcess() async throws -> ResourceHandle<Never> {
        ...
        guard projectedCount <= 100 else {  // Conservative limit
            await cancelReservation(reservation.value)
            throw SafetyResourceError.limitExceeded(type: .process, requested: 1)
        }
    ...
    func confirmReservation(_ handle: ReservationHandle) async throws -> ResourceHandle<Never> {
        ...
        guard reservation.isActive.compareExchange(
            ...
            throw SafetyResourceError.reservationInactive(id: reservation.id)
        }
...
public enum SafetyResourceError: LocalizedError {
    case shutdownInProgress
    case limitExceeded(type: SafetyResourceType, requested: Int)
    case reservationInactive(id: UUID)
    
    public var errorDescription: String? {
        switch self {
        case .shutdownInProgress:
            return "Cannot allocate resources during shutdown"
        case .limitExceeded(let type, let requested):
            return "Resource limit exceeded for \(type): requested \(requested)"
        case .reservationInactive(let id):
            return "Reservation \(id) is no longer active"
        }
    }
}
