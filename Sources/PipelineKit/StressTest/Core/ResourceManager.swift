...
public actor ResourceManager {
    ...
    public func register(
        ...
        guard !isCleaningUp else {
            throw ResourceError.managerShuttingDown
        }
        ...
        case .memory:
            if let size = size, let limit = limits.maxMemory {
                guard usage.memoryBytes + size <= limit else {
                    throw ResourceError.limitExceeded(
                        type: type,
                        requested: size,
                        limit: limit,
...
public enum ResourceError: LocalizedError {
    case limitExceeded(type: ResourceType, requested: Int, limit: Int, current: Int)
    case notFound(UUID)
    case managerShuttingDown
    
    public var errorDescription: String? {
        switch self {
        case .limitExceeded(let type, let requested, let limit, let current):
            return "Resource limit exceeded for \(type): requested \(requested), limit \(limit), current \(current)"
        case .notFound(let id):
            return "Resource not found: \(id)"
        case .managerShuttingDown:
            return "Resource manager is shutting down"
        }
    }
}
