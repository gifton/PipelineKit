...
/// Errors related to back-pressure and capacity management.
public enum BackPressureError: Error, Sendable {
    /// The pipeline has reached its maximum outstanding command limit.
    case queueFull(current: Int, limit: Int)
    
    /// A timeout occurred while waiting for capacity to become available.
...
extension BackPressureError: LocalizedError {
    public var errorDescription: String? {
        ...
        case let .queueFull(current, limit):
            return "Pipeline queue is full (\(current)/\(limit))"
