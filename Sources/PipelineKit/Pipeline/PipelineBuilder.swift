...
///
/// `PipelineBuilder` provides a convenient way to construct a `PipelineExecutor` with
...
/// - Build a fully configured pipeline executor
///
/// ## Example
/// ```swift
/// // Create a handler
...
public final class PipelineBuilder<T: Command, H: CommandHandler> where H.CommandType == T {
    ...
    /// Builds and returns a configured `PipelineExecutor`.
    ///
    /// This method creates a new `PipelineExecutor` with the configured handler,
    /// middleware, and maximum depth. All middleware are added to the executor
    /// before it is returned.
    ///
    /// - Returns: A fully configured `PipelineExecutor` ready for use.
    /// - Throws: `PipelineError.maxDepthExceeded` if the total number of middleware exceeds the maximum depth.
    ///
    /// - Note: This method is async because adding middleware to the executor requires async operations.
    public func build() async throws -> PipelineExecutor {
        let executor = PipelineExecutor(handler: handler, maxDepth: maxDepth)
        try await executor.addMiddlewares(middlewares)
        return executor
    }
}