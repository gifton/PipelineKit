...
public enum ExportError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .notConfigured:
            return "Exporter is not properly configured"
        case .destinationUnavailable(let dest):
            return "Export destination unavailable: \(dest)"
        case .serializationFailed(let reason):
            return "Failed to serialize metrics: \(reason)"
        case .ioError(let error):
            return "I/O error: \(error.localizedDescription)"
        case .quotaExceeded:
            return "Export quota exceeded"
        case .shutdownInProgress:
