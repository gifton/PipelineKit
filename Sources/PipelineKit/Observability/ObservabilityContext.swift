...
// MARK: - Observable Command Protocol

/// A protocol for commands that want to emit custom observability events
...
public extension ObservableCommand {
    func setupObservability(context: CommandContext) async {
...
// MARK: - Observability Middleware Protocol

/// A protocol for middleware that wants to participate in observability
public protocol ObservableMiddleware: Middleware {
    /// The name used for observability tracking
    var observabilityName: String { get }
    
    /// Custom tags to include in observability events
    var observabilityTags: [String: String] { get }
}

public extension ObservableMiddleware {
