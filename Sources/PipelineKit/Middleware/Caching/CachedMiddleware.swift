...
public actor InMemoryMiddlewareCache: MiddlewareCache {
    ...
    public func getStats() -> CacheStatistics {
...
// MARK: - Cache-aware Middleware Protocol

/// Protocol for middleware that can provide caching hints
public protocol CacheAwareMiddleware: Middleware, Sendable {
    /// Whether results from this middleware can be cached
    var isCacheable: Bool { get }
    
    /// Suggested TTL for cached results
    var suggestedTTL: TimeInterval { get }
    
    /// Generate a cache key for the given command
    func cacheKey<T: Command>(for command: T, context: CommandContext) -> String?
}

extension CacheAwareMiddleware {
    public var isCacheable: Bool { true }
    public var suggestedTTL: TimeInterval { 300 } // 5 minutes default
}
