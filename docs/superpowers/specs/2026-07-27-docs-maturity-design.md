...
*(Amended 2026-07-30 after Tier 3 brainstorm: exploration found root `SECURITY.md` contains no vulnerability-disclosure policy at all — it is a 1,091-line best-practices guide — and GitHub private vulnerability reporting is disabled on the repo. The original "accuracy pass" line item becomes a split.)*

- **SECURITY.md split — policy + guide**: root `SECURITY.md` becomes a short, standard security policy: supported versions (honestly: latest 0.x release only), how to report (GitHub private vulnerability reporting via the repo's Security tab — enabling that repo setting via the API is a step of this tier), what a reporter can expect, and scope. The 1,091-line best-practices content moves via history-preserving `git mv` to `docs/guides/security-best-practices.md` and gets the deepest accuracy pass of the program there: all ~30 code blocks harness-compiled against real API (Tier 2 methodology); known rot fixed (six stale `metadata: CommandMetadata` middleware signatures, nonexistent `HTTPCommandMetadata` ~line 518); false process claims replaced with what CI verifiably does (the referenced `.github/workflows/dependency-audit.yml` does not exist; "exact version pinning" is false — all four real deps use `from:`; SBOM/weekly/monthly-audit claims verified against the real workflow files or removed); the broken `../DEPENDENCIES.md` link corrected. Fabricated capabilities are deleted, not hedged.
- **`VERSIONING.md` — honest minimal**: patch (0.5.x) = fixes/docs only, no source breaks; minor (0.x) may break source, always documented in CHANGELOG's Breaking section; no release-cadence promise; 1.0 stated as criteria, not a date; the untagged-1.0.0 anomaly explained factually, cross-linked from the existing CHANGELOG 1.0.0 note. Explicitly no deprecation-window commitment.
- **Platform support statement** (README section + doc): exact supported OS versions from `Package.swift` (iOS/macOS/tvOS/watchOS/visionOS 26.0+, Swift 6.2), the *why* (strict-concurrency adoption), and Linux represented exactly as CI treats it (build-verified vs tested — verified against workflow files at plan time). Floor changes are CHANGELOG'd; no promises about floors we don't ship.
- **Enterprise evaluation guide** (`docs/guides/enterprise-evaluation.md`): the 30-minute POC path (install with a real tag → BasicExample → one middleware → observability hookup), integration patterns, a stable-vs-newer-surface map (grounded in VERSIONING.md), and where to report what — bugs to GitHub issues, vulnerabilities to the security policy. Every code block harness-compiled.
- **File the four Tier 2 parked code bugs as GitHub issues** (CommandContext metrics recorders discarding name/value/tags; `BulkheadMiddleware.tagged` sharing one semaphore across tags; HealthCheck failure recording ignoring error identity + `checkFailed` never constructed; `ConcurrentPipeline`'s unreachable timeout throw), using the evidence gathered in the Tier 2 review. Doc caveats and the evaluation guide link the real issue numbers. Fixing them stays out of scope.
- **Tier constraint**: no `Sources/` changes at all (docs, scripts, repo settings only) — filtered test suites suffice; the full-Xcode-suite gate does not apply to this tier.
